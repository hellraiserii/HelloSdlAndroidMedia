...
public class SdlService extends Service implements IProxyListenerALM {
    ...
    public void sendCommands1() {
        ...
        command.setVrCommands(Arrays.asList(TEST_COMMAND_NAME1));
    ...
    public void sendCommands2() {
        ...
        command.setMenuParams(params);
        command.setVrCommands(Arrays.asList(TEST_COMMAND_NAME2));
        sendRpcRequest(command);
    }
    ...
    public void sendCommands3() {
        ...
        command.setMenuParams(params);
        command.setVrCommands(Arrays.asList(TEST_COMMAND_NAME3));
        sendRpcRequest(command);
    }
    ...
    public void sendCommands4() {
        ...
        command.setMenuParams(params);
        command.setVrCommands(Arrays.asList(TEST_COMMAND_NAME4));
        sendRpcRequest(command);
    }
    ...
    public void onGetVehicleDataResponse(GetVehicleDataResponse response) {
        ...
        try {
            if (response.getSpeed() != null) {
                ...
                Log.i(TAG, "is Driver Braking??? " + response.getDriverBraking() + " speed: " + speed);
                String gpsCoords = null;
                gpsCoords = (response.getGps() != null ? response.getGps().toString() : "ND");
                Log.w(TAG, "GPS: " + gpsCoords);

                try {
                    String data = "speed: " + (response.getSpeed() != null ? response.getSpeed().toString() : "ND")
                            + " gps: " + (response.getGps() != null ? response.getGps().toString() : "ND")
                            + " driverIsBraking: " + (response.getDriverBraking() != null ? response.getDriverBraking().toString() : "ND");

                    Log.i(TAG, data);

                    proxy.show("Datos:", data, null, autoIncCorrId++);
            ...
            if (response.getDriverBraking() == VehicleDataEventStatus.YES) {
                String gpsCoords = null;
                gpsCoords = (response.getGps() != null ? response.getGps().toString() : "ND");
                Log.w(TAG, "ESTAMOS FRENANDOOOOO - GPSCOORDS: " + gpsCoords + "- ESTARÍAMOS LOGUEANDO ESTE EVENTO + GPS EN CLOUD");
    ...
    public void onOnDriverDistraction(OnDriverDistraction notification) {
        Log.i("TAG", "DRIVER DISTRACTION " + notification);
    }
